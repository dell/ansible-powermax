...
class MaskingView(object):
    ...
    def is_mv_changed(self, mv):
        ...
        is_mv_changed = False
        if 'portGroupId' in mv and self.module_wo_senstive_data['portgroup_name'] is \
                not None and mv['portGroupId'] != \
                self.module_wo_senstive_data['portgroup_name']:
            is_mv_changed = True
        elif 'storageGroupId' in mv and self.module_wo_senstive_data['sg_name'] \
                is not None and mv['storageGroupId'] \
                != self.module_wo_senstive_data['sg_name']:
            is_mv_changed = True
        elif 'hostId' in mv and self.module_wo_senstive_data['host_name'] is not None \
                and mv['hostId'].lower() != self.module_wo_senstive_data['host_name'].lower():
            is_mv_changed = True
        elif 'hostGroupId' in mv and self.module_wo_senstive_data['hostgroup_name'] \
                is not \
                None and mv['hostGroupId'].lower() != \
                self.module_wo_senstive_data['hostgroup_name'].lower():
            is_mv_changed = True
        elif 'hostId' in mv and self.module_wo_senstive_data['hostgroup_name'] \
                is not None:
            is_mv_changed = True
        elif 'hostGroupId' in mv and self.module_wo_senstive_data['host_name'] \
                is not None:
            is_mv_changed = True
        if is_mv_changed:
    ...
    def create_masking_view(self, mv_name):
        ...
        pg_name = self.module_wo_senstive_data['portgroup_name']
        sg_name = self.module_wo_senstive_data['sg_name']
        host_name = self.module_wo_senstive_data['host_name']
        hostgroup_name = self.module_wo_senstive_data['hostgroup_name']
        starting_lun_address = \
            self.module_wo_senstive_data['starting_lun_address'] if self.module_wo_senstive_data['starting_lun_address'] else None
    ...
    def perform_module_operation(self):
        ...
        mv_name = self.module_wo_senstive_data['mv_name']
        new_mv_name = self.module_wo_senstive_data['new_mv_name']
