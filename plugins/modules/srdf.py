...
class SRDF(object):
    ...
    def get_srdf_link(self, sg_name):
        ...
        '''
        srdf_link_details = []
        rdfg_number = self.module_wo_senstive_data['rdfg_no']
        try:
            if rdfg_number:
    ...
    def create_srdf_link(self):
        ...
        remote_serial_no = self.module_wo_senstive_data['remote_serial_no']
        srdf_mode = self.module_wo_senstive_data['srdf_mode']
        ...
        try:
            establish_flag = self._compute_required_establish_flag(
                self.module_wo_senstive_data['srdf_state'])
            rdfg_number = self.module_wo_senstive_data['rdfg_no']
            async_flag = not (self.module_wo_senstive_data['wait_for_completion'])
            witness = self.module_wo_senstive_data['witness']
            ...
            if not self.module.check_mode:
                if self.module_wo_senstive_data['wait_for_completion'] is False:
                    resp = self.replication.create_storage_group_srdf_pairings(
                        ...
                        _async=async_flag)
                elif self.module_wo_senstive_data['wait_for_completion'] is True:
    ...
    def modify_srdf_state(self, action):
        ...
        if self.module_wo_senstive_data['witness'] is not None:
            if srdf_link['modes'][0] != 'Active':
                ...
                modify_body['options'] = {
                    action.lower(): {
                        'metroBias': not (self.module_wo_senstive_data['witness'])}}
    ...
    def check_for_multiple_rdf_groups(self, srdf_link=None, get_only=True):
        ...
        storage group.
        '''
        rdfg_no = self.module_wo_senstive_data['rdfg_no']
        found = False
        return_rdfg = None
    ...
    def perform_module_operation(self):
        ...
        sg_name = self.module_wo_senstive_data['sg_name']
        srdf_mode = self.module_wo_senstive_data['srdf_mode']
        srdf_state = self.module_wo_senstive_data['srdf_state']
        job_id = self.module_wo_senstive_data['job_id']
        changed = False
        remoteSymmetrixIDs = []
        ...
        if job_id:
            ...
            if state == 'present':
                ...
                if self.result['SRDF_link_details']\
                        and self.module_wo_senstive_data['remote_serial_no'] \
                        and not changed:
