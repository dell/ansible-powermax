...
class MetroDR(object):
    ...
    def __init__(self):
        ...
        self.module_wo_senstive_data = utils.get_powermax_management_host_parameters_remove_sensitive_data(self.module.params)
    ...
    def get_metrodr_env(self):
        ...
        try:
            LOG.info("Getting metro DR environment details for: %s",
                     self.module_wo_senstive_data['env_name'])
            ...
            error_message = "Failed to get details of environment " \
                            "{0} with error {1}" \
                .format(self.module_wo_senstive_data['env_name'], str(e))
    ...
    def pre_checks_for_create(self):
        ...
        for k in ("metro_r2_array_id", "dr_array_id", "replication_mode"):
            if not self.module_wo_senstive_data[k]:
    ...
    def create_metrodr_env(self):
        ...
        try:
            ...
            param = {
                "storage_group_name": self.module_wo_senstive_data["sg_name"],
                "environment_name": self.module_wo_senstive_data["env_name"],
                "metro_r1_array_id": self.module_wo_senstive_data["metro_r1_array_id"],
                "metro_r2_array_id": self.module_wo_senstive_data["metro_r2_array_id"],
                "dr_array_id": self.module_wo_senstive_data["dr_array_id"],
                "dr_replication_mode": self.module_wo_senstive_data["replication_mode"],
                "force_new_metro_r1_dr_rdfg": self.module_wo_senstive_data[
                    "new_rdf_group_r1"],
                "force_new_metro_r2_dr_rdfg": self.module_wo_senstive_data[
                    "new_rdf_group_r2"],
                "_async": not self.module_wo_senstive_data['wait_for_completion']
            }
            resp = {}
            ...
            LOG.info("Creating metro DR environment: %s param: %s",
                     self.module_wo_senstive_data['env_name'], param)
    ...
    def get_storage_group_srdf_details(self, srdf_gr):
        ...
        try:
            ...
            srdf_detail = self.replication.get_storage_group_srdf_details(
                self.module_wo_senstive_data['sg_name'], srdf_gr)
    ...
    def pre_checks_for_convert(self):
        ...
        if not srdf_gr_list:
            self.show_error_exit(
                "SG: %s does not have srdf group" % self.module_wo_senstive_data[
                    'sg_name'])
        ...
        for srdf_gr in srdf_gr_list:
            ...
            if "Adaptive Copy" in srdf_detail['modes'] or "Asynchronous" in\
                ...
                if self.module_wo_senstive_data["dr_array_id"]:
                    LOG.info("Validating given dr_array_id")
                    if self.get_rdf_group(srdf_gr)["remoteSymmetrix"] \
                            == self.module_wo_senstive_data["dr_array_id"]:
            ...
            # precheck: 3
                ...
                if self.module_wo_senstive_data["metro_r2_array_id"]:
                    LOG.info("Validating given metro_r2_array_id")
                    if self.get_rdf_group(srdf_gr)["remoteSymmetrix"] \
                            == self.module_wo_senstive_data["metro_r2_array_id"]:
    ...
    def pre_checks_for_modify(self):
        ...
        if self.module_wo_senstive_data['replication_mode'] and self.module_wo_senstive_data[
        ...
        if self.module_wo_senstive_data['srdf_param']['keep_r2']:
            if self.module_wo_senstive_data['srdf_param']['srdf_state'] \
    ...
    def get_modify_dict(self, metrodr_env_details):
        ...
        if self.module_wo_senstive_data["replication_mode"]:
            if self.module_wo_senstive_data["replication_mode"] != \
                ...
                if self.module_wo_senstive_data["replication_mode"] == \
                    ...
                    modify_dict["dr_replication_mode"] = \
                        self.module_wo_senstive_data["replication_mode"]
            else:
                LOG.info("Replication mode is already: %s",
                         self.module_wo_senstive_data['replication_mode'])

        # comparing srdf_state
        if self.module_wo_senstive_data["srdf_param"]["srdf_state"]:  # mandatory param
            # dr-true, metro-true
            if self.module_wo_senstive_data["srdf_param"]["metro"] \
                    and self.module_wo_senstive_data["srdf_param"]["dr"]:
                # Idempotency check for 'Establish', 'Suspend' and 'Recover'
                # states
                if ((self.module_wo_senstive_data["srdf_param"]["srdf_state"] in
                        IDEMPOTENCY_STATES[
                            metrodr_env_details["dr_state"]])
                    and (self.module_wo_senstive_data["srdf_param"]["srdf_state"] in
                         IDEMPOTENCY_STATES[
                    ...
                    modify_dict["action"] = \
                        self.module_wo_senstive_data["srdf_param"]["srdf_state"]
            # dr-false, metro-false --> No change in states
            elif not self.module_wo_senstive_data["srdf_param"]["metro"] \
                    and not self.module_wo_senstive_data["srdf_param"]["dr"]:
                LOG.info("Given srdf_state is already maintained by "
                         "DR and Metro arrays")
            elif self.module_wo_senstive_data["srdf_param"]["metro"]:
                if metrodr_env_details["metro_state"] in IDEMPOTENCY_STATES:
                    ...
                    self.show_error_exit(
                        ...
                        % metrodr_env_details["metro_state"])
            elif self.module_wo_senstive_data["srdf_param"]["dr"]:
    ...
    def modify_metrodr_env(self, metrodr_env_details):
        ...
        """
        if not self.module_wo_senstive_data["srdf_param"] \
                or not self.module_wo_senstive_data["srdf_param"]["srdf_state"]:
        ...
        modify_dict["metro"] = self.module_wo_senstive_data["srdf_param"]["metro"]
        modify_dict["dr"] = self.module_wo_senstive_data["srdf_param"]["dr"]
        modify_dict["keep_r2"] = self.module_wo_senstive_data["srdf_param"]["keep_r2"]
        modify_dict["_async"] = self.module_wo_senstive_data["wait_for_completion"]
        ...
        try:
            ...
            if (modify_dict["action"] == "SetMode"
                    and self.module_wo_senstive_data['replication_mode']
                    == metrodr_env_details["dr_rdf_mode"]):
                LOG.info(
            ...
            if self.module_wo_senstive_data["wait_for_completion"]:
        ...
        try:
            ...
            if not self.module.check_mode:
                self.metro.delete_metrodr_environment(
                    self.module_wo_senstive_data['env_name'],
                    self.module_wo_senstive_data['remove_r1_dr_rdfg'])
    ...
    def perform_module_operation(self):
        ...
        if self.module_wo_senstive_data['state'] == "present":
        ...
        elif self.module_wo_senstive_data['state'] == "absent":
