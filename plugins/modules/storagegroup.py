...
class StorageGroup(object):
    ...
    def get_volume(self):
        """Get volume details"""
        vol_id = self.module_wo_senstive_data['vol_id']
        if vol_id is None:
            vol_id = self.provisioning.find_volume_device_id(
                self.module_wo_senstive_data['vol_name'])
        try:
    ...
    def create_storage_group(self, sg_name):
        ...
        slo = self.module_wo_senstive_data['service_level']
        srp = self.module_wo_senstive_data['srp']
        compression = self.module_wo_senstive_data['compression']
        ...
        resp = {}
        try:
            if self.module_wo_senstive_data['snapshot_policies'] \
                    and self.module_wo_senstive_data['snapshot_policy_state'] \
                    == 'present-in-group':
                self.pre_check_for_PyU4V_version()
                snapshot_policies = self.module_wo_senstive_data['snapshot_policies']
    ...
    def move_volume_between_storage_groups(self, sg_name, target_sg_name, force):
        ...
        :return: boolean
        """
        volumes = self.module_wo_senstive_data['volumes']
        vol_state = self.module_wo_senstive_data['vol_state']
                ...
                try:
                    ...
                    if not volume_list or len(volume_list) == 0 or name is None:
                        ...
                        if self.if_srdf_protected(storage_group):
                            array_id = self.module_wo_senstive_data['serial_no']
    ...
    def modify_sg_slo(self, sg_name, new_slo):
        ...
        if ('slo' in storage_group and storage_group['slo'].lower() !=
            self.module_wo_senstive_data['service_level'].lower()) or \
                ('slo' not in storage_group and
                 self.module_wo_senstive_data['service_level'].lower() != "none"):
            try:
                LOG.info('The existing SG SLO is %s and the desired SG SLO '
                         'is %s ', storage_group['slo'],
                         self.module_wo_senstive_data['service_level'])
                if not self.module.check_mode:
        ...
        if 'compression' in storage_group and storage_group[
            ...
            try:
                LOG.info('The existing SG compression is %s and the desired'
                         ...
                         self.module_wo_senstive_data['compression'])
                if not self.module.check_mode:
                    self.provisioning.modify_storage_group(sg_name, payload)
            except Exception as e:
    ...
    def is_storage_group_modified(self, storage_group):
        ...
        if ('srp' in storage_group and self.module_wo_senstive_data['srp'] is not None
            and storage_group['srp'].lower() != self.module_wo_senstive_data['srp'].
            lower()) or ('srp' not in storage_group and self.module.
                         params['srp'] is not None and self.module_wo_senstive_data['srp'].
                         lower() != "none"):
            modified_param['modified_srp'] = self.module_wo_senstive_data['srp']
            modified = True

        if ('slo' in storage_group and self.module_wo_senstive_data['service_level'] is
    ...
    def rename_storage_group(self, storage_group, new_sg_name):
        ...
        if not storage_group:
            error_message = ("Rename storage group %s to new name %s "
                             ...
                             % (self.module_wo_senstive_data['sg_name'], new_sg_name))
            self.show_error_exit(msg=error_message)
        changed = False
        ...
        try:
            ...
            error_message = ("Rename storage group %s to new name %s "
                             "failed with error %s "
                             % (self.module_wo_senstive_data['sg_name'], new_sg_name, str(e)))
            self.show_error_exit(msg=error_message)
        return changed
    ...
    def perform_module_operation(self):
        ...
        chosen in playbook
        """
        state = self.module_wo_senstive_data['state']
        sg_name = self.module_wo_senstive_data['sg_name']
        target_sg_name = self.module_wo_senstive_data['target_sg_name']
        force = self.module_wo_senstive_data['force'] or False
        volumes = self.module_wo_senstive_data['volumes']
        vol_state = self.module_wo_senstive_data['vol_state']
        child_sgs = self.module_wo_senstive_data['child_storage_groups']
        child_sg_state = self.module_wo_senstive_data['child_sg_state']
        new_sg_name = self.module_wo_senstive_data['new_sg_name']
        snapshot_policies = self.module_wo_senstive_data['snapshot_policies']
        snapshot_policy_state = self.module_wo_senstive_data['snapshot_policy_state']
        host_io_limit = self.module_wo_senstive_data['host_io_limit']
