...
class Info(object):
    ...
    def __init__(self):
        ...
        temp.pop('password')
        self.module_wo_senstive_data = temp
        serial_no = self.module_wo_senstive_data['serial_no']
    ...
    def get_volume_list(self, tdev_volumes=False, filters_dict=None):
        ...
        try:
            ...
            array_serial_no = self.module_wo_senstive_data['serial_no']
            if tdev_volumes:
                if filters_dict:
    ...
    def get_volume_details(self, vol_list):
        ...
        try:
            ...
            msg = 'Get Volumes for array %s failed with error %s '\
                  % (self.module_wo_senstive_data['serial_no'], str(e))
            self.show_error_exit(msg=msg)
    ...
    def show_error_exit(self, msg):
        if self.u4v_conn is not None:
            try:
                LOG.info("Closing Unisphere connection %s", self.module_wo_senstive_data['serial_no'])
    ...
    def perform_module_operation(self):
        ...
        if serial_no == '':
            ...
            subset = self.module_wo_senstive_data['gather_subset']
            tdev_volumes = self.module_wo_senstive_data['tdev_volumes']
            masking_view_name = self.module_wo_senstive_data['masking_view_name']
            filters = []
            filters = self.module_wo_senstive_data['filters']
            if len(subset) == 0:
...
def get_info_parameters_without_senstive_data():
    """This method provide the parameters required for the Ansible
    modules on PowerMax"""
