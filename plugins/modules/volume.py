...
class Volume(object):
    ...
    def get_volume(self):
        ...
        volume_list = None
        volume_name = None
        if self.module_wo_senstive_data['vol_name'] is not None:
            volume_name = self.module_wo_senstive_data['vol_name']
            params = {"storageGroupId": sg_name,
                      "volume_identifier": volume_name}
            volume_list = self.provisioning.get_volume_list(params)

        if self.module_wo_senstive_data['vol_wwn'] is not None:
            vol_wwn = self.module_wo_senstive_data['vol_wwn']
        ...
        if not volume_list or len(volume_list) == 0:
            ...
            if self.module_wo_senstive_data['vol_wwn'] is not None and "effective_wwn" \
                    in vol and vol['effective_wwn'] \
                    != self.module_wo_senstive_data['vol_wwn']:
                msg = ('The given volume with WWN: %s also has an effective'
                       ...
                       % (self.module_wo_senstive_data['vol_wwn'], vol['effective_wwn']))
    ...
    def create_volume(self, vol_name, sg_name, size, cap_unit):
        ...
        try:
            if self.module_wo_senstive_data['vol_name'] is None:
    ...
    def get_created_vol_id(self, job, vol_name, sg_name):
        ...
        try:
            append_vol_id = self.module_wo_senstive_data['append_vol_id'] or False
            vol_id = None
            if append_vol_id:
    ...
    def get_create_volume_params(self, sg_name=None, num_vols=None,
        ...
        append_vol_id = self.module_wo_senstive_data['append_vol_id']
    ...
    def perform_module_operation(self):
        ...
        chosen in playbook
        """
        size = self.module_wo_senstive_data['size']
        state = self.module_wo_senstive_data['state']
        new_name = self.module_wo_senstive_data['new_name']
        vol_id = self.module_wo_senstive_data['vol_id']
        vol_name = self.module_wo_senstive_data['vol_name']
        sg_name = self.module_wo_senstive_data['sg_name']
        cap_unit = self.module_wo_senstive_data['cap_unit']
