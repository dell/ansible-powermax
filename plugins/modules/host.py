...
class Host(object):
    ...
    def create_host(self, host_name, initiators):
        ...
        '''
        received_host_flags = self.module_wo_senstive_data['host_flags']
        host_type = self.module_wo_senstive_data['host_type']
    ...
    def _create_result_dict(self, changed, host=None):
        ...
        if self.module_wo_senstive_data['state'] == 'absent' or \
                (not host and self.module.check_mode):
            self.result['host_details'] = {}
        else:
            if self.module_wo_senstive_data['new_name'] and not self.module.check_mode:
                self.result['host_details'] = self.get_host(
                    self.module_wo_senstive_data['new_name'])
            else:
                self.result['host_details'] = self.get_host(
                    self.module_wo_senstive_data['host_name'])
    ...
    def perform_module_operation(self):
        ...
        initiator_state = self.module_wo_senstive_data['initiator_state']
        host_name = self.module_wo_senstive_data['host_name']
        initiators = self.module_wo_senstive_data['initiators']
        new_name = self.module_wo_senstive_data['new_name']
        host_flags = self.module_wo_senstive_data['host_flags']
        host_type = self.module_wo_senstive_data['host_type']
